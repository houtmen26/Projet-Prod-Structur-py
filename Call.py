...
class Call(ProduitDerive):
    ...
    def prix_black_scholes(self, date: datetime = None):
        ...
        prix_call = S0 * norm.cdf(d1) - self.strike * exp(-self.taux_sans_risque * self.maturite) * norm.cdf(d2)
        
        return prix_call * self.nominal
    ...
    def prix_caplet(self, date: datetime = None):
        ...
        # Calcul des paramètres de Black-Scholes pour le caplet
        d1 = (log(F / self.strike) + (0.5 * self.volatilite ** 2) * self.maturite) / (self.volatilite * sqrt(self.maturite))
        d2 = d1 - self.volatilite * sqrt(self.maturite)
        
        # Calcul du prix du caplet avec la formule de Black-Scholes
        prix_caplet = exp(-self.taux_sans_risque * self.maturite) * (F * norm.cdf(d1) - self.strike * norm.cdf(d2))
        
        return prix_caplet * self.nominal
    ...
    def description(self):
        ...
        """
        return f"Option Call: {self.nom}, Strike: {self.strike}, Volatilité: {self.volatilite*100:.2f}%, " \
